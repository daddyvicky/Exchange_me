<?php
			...
			if($res && $res->num_rows === 1) {
				...
				if($user['verified'] === 0) {
					if($user['COUNT(requests.id)'] <= MAX_EMAIL_VERIFICATION_REQUESTS_PER_DAY) {
						...
						if($requestID !== -1) {
							if(sendEmail($email, $user['name'], 'Email Verification', '<a href="http://localhost/exchange/validate/'.$requestID.'/'.urlSafeEncode($verifyCode).'">Click this link to verify your email</a>')) {
								return 0;
							}
							else {
	...
	if(isset($_POST['email']) && isset($_POST['csrf_token']) && validateToken($_POST['csrf_token'])) {
		echo sendValidationEmail($_POST['email']);
	}
